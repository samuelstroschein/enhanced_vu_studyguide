...
for course_id in course_ids:
    ...
    # name of the course
    g.add((URIRef(VUC + course_id), TEACH.courseTitle, Literal(soup.find(id="title").find("h2").contents[0])))
    ...
    g.add((URIRef(VUC + course_id), TEACH.ects, Literal(int(general_information[1].split()[0]))))
    # academicTerm is string because can be "whole academic year" or "P(eriod)3"
    g.add((URIRef(VUC + course_id), TEACH.academicTerm, Literal(general_information[2])))
    # just using VU.course level because I could not find something on the vocabularies
    g.add((URIRef(VUC + course_id), VUP.course_level, Literal(int(general_information[3]))))
    # language is using the dbr recourse 
    language_dbr_format = general_information[4].split()[0] + '_language'
    g.add((URIRef(VUC + course_id), DBO.language, URIRef(DBR + language_dbr_format)))
    ...
    teachers = (general_information[6] + general_information[8]).split('\n')[:-1]
